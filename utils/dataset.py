...
class MixDataset(torch.utils.data.Dataset):
    def __init__(self, dataset, mixer, classA, classB, classC,
        ...
        self.n_normal = int(L * normal_rate* data_rate)
        self.n_mix = int(L * mix_rate* data_rate)
        self.n_poison = int(L * poison_rate* data_rate)
...
def pick_three_numbers(exclude):
    numbers = list(range(10))  # 创建一个0-9的列表
    numbers.remove(exclude)  # 从列表中移除数字c
    return random.sample(numbers, 3)
    
class MixDataset_rand(torch.utils.data.Dataset):
    def __init__(self, dataset, mixer, classA, classB, classC,
                 data_rate, normal_rate, mix_rate, poison_rate,
                 transform=None):
        """
    ...
    def poison_item(self):
        a, b, c = pick_three_numbers(self.classC)
        # (img1, classA)
        index1 = self.random_choice(self.uni_index[a])
        img1, _, args1 = self.basic_item(index1)
        # (img2, classB)
        index2 = self.random_choice(self.uni_index[b])
        img2, _, args2 = self.basic_item(index2)
        return img1, img2, c, args1, args2
