...
class GoogLeNet(nn.Module):
    def __init__(self, num_classes=NUM_CLASSES):
        ...
        self.pre_layers = BasicConv2d(3, 192, kernel_size=3, padding=1) # 32*32*192

        self.a3 = Inception(192, 64, 96, 128, 16, 32, 32) # 32*32* 256(64+128+32+32)
        self.b3 = Inception(256, 128, 128, 192, 32, 96, 64) # 32*32* 480(128+192+96+64)

        self.max_pool3 = nn.MaxPool2d(3, stride=2, padding = 1) # 16*16 *480

        self.a4 = Inception(480, 192, 96, 208, 16, 48, 64) # 16*16* 512(192+208+48+64)
        self.b4 = Inception(512, 160, 112, 224, 24, 64, 64) # 16*16* 512
        self.c4 = Inception(512, 128, 128, 256, 24, 64, 64) # 16*16* 512
        self.d4 = Inception(512, 112, 144, 288, 32, 64, 64) # 16*16* 528
        self.e4 = Inception(528, 256, 160, 320, 32, 128, 128) # 16*16* 832

        # Pool
        self.a5 = Inception(832, 256, 160, 320, 32, 128, 128) # 8*8* 832
        self.b5 = Inception(832, 384, 192, 384, 48, 128, 128) # 8*8* 1024(384+384+128+128)

        self.avg_pool = nn.AvgPool2d(8, stride = 1) # 1*1*1024
