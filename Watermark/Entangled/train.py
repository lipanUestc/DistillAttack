...
def get_args_parser():
    ...
    parser.add_argument('-c', '--config', default='configs/cifar10.json', type=str)
...
def get_our_loader(train_set, val_set):
    ...
    extended_train_loader = torch.utils.data.DataLoader(dataset=extended_train_data, 
                                                        batch_size=args.batch_size,
                                                        shuffle=True,
                                                        num_workers=4,
                                                        drop_last=True)

    return extended_train_loader
...
def main(args):
    ...
    adv_loader = get_our_loader(train_dataset, test_dataset)
...
def prepare_dataset_np(args):
    ...
    if dataset == 'mnist' or dataset == 'fashion':
        ...
        test_transforms = transforms.Compose(transform_list)
        if dataset == 'cifar10':
            train_dataset = datasets.CIFAR10('/home/lpz/MHL/data/', train=True, transform=train_transforms, download=True)
            test_dataset = datasets.CIFAR10('/home/lpz/MHL/data/', train=False, transform=test_transforms, download=True)
        else:
            train_dataset = datasets.CIFAR100('/home/lpz/MHL/data/', train=True, transform=train_transforms, download=True)
            test_dataset = datasets.CIFAR100('/home/lpz/MHL/data/', train=False, transform=test_transforms, download=True)
