...
encoder = models.__dict__[args.arch](num_classes=args.num_classes)
encoder = teacher_model.to(device)
    
checkpoint = torch.load("/home/lpz/MHL/model-watermarking-main/checkpoint/frontierstitching_cifar10_custom_cnn_SGD_MultiStepLR_100.pth")
...
if method == 'PCA':
    data_new = PCA(n_components=3, random_state = 9).fit_transform(datas)
else:
...
plt.savefig('ColorLab/teacher_rob.jpg')
plt.show()
