...
if device == 'cpu':
    ...
    net.load_state_dict(torch.load(args.loadmodel)['net'])#os.path.join(cwd, 'checkpoint', args.loadmodel + '.pth')))
...
train_db_path = "/home/lpz/MHL/data/"
test_db_path = "/home/lpz/MHL/data/"
...
elif args.attack_type == 'valid':

    if device == 'cpu':
        net.load_state_dict(
            torch.load(args.loadmodel, map_location=torch.device('cpu')))
    else:
        net.load_state_dict(torch.load(args.loadmodel)['net'])#os.path.join(cwd, 'checkpoint', args.loadmodel + '.pth')))
